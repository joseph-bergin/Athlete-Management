...
import { ParsedAthleteData, ParsedAthleteDataDTO } from './models';
import { Team } from '@/providers/team.provider';

type CatapultCsvUploadProps = {
  selectedTeam: Team | undefined;
  setCsvData: React.Dispatch<React.SetStateAction<ParsedAthleteData[]>>;
  setLoading: React.Dispatch<React.SetStateAction<boolean>>;
};

const CatapultUpload: React.FC<CatapultCsvUploadProps> = ({ selectedTeam, setCsvData, setLoading }) => {
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      const data: ParsedAthleteData[] = XLSX.utils.sheet_to_json(sheet, { dateNF: 'd"/"m"/"yyyy' });
      const dto: ParsedAthleteDataDTO[] = data.map((entry) => ({ ...entry, teamID: selectedTeam?.teamID }));
