...
import { ParsedForceFrameAthleteData, ParsedForceFrameAthleteDataDTO } from './models';
import { Team } from '@/providers/team.provider';

type ForceFrameCsvUploadProps = {
  selectedTeam: Team | undefined;
  setForceCsvData: React.Dispatch<React.SetStateAction<ParsedForceFrameAthleteData[]>>;
  setLoading: React.Dispatch<React.SetStateAction<boolean>>;
};

const ForceFrameUpload: React.FC<ForceFrameCsvUploadProps> = ({ selectedTeam, setForceCsvData, setLoading }) => {
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      const data: ParsedForceFrameAthleteData[] = XLSX.utils.sheet_to_json(sheet, { dateNF: 'd"/"m"/"yyyy' });
      const dto: ParsedForceFrameAthleteDataDTO[] = data.map((entry) => ({ ...entry, teamID: selectedTeam?.teamID }));
  ...
  const uploadCsvData = async (data: ParsedForceFrameAthleteDataDTO[]) => {
    try {
      setLoading(true);
